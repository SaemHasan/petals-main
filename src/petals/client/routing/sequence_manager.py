...
# latency ==> delay = self._rtt_to_delay(rtts.get(span.peer_id))
...
class RemoteSequenceManager:
    ...
    def make_sequence(
        ...
        *,
        ...
        print(f"\n======Making sequence from {start_index} to {end_index} with mode {mode}===========\n")
        ...
        elif mode == "latency_throughput_tradeoff":
            sequence_problem = ChainSequence(n_blocks=end_index - start_index, n_servers=len(self.state.sequence_info.spans_by_priority), rtts=self.ping_aggregator.to_dict(), throughputs=self.state.sequence_info.spans_by_priority)
            mutation = BitflipMutation(0.1)
            crossover = SinglePointCrossover(0.9)
            sampling = BinaryRandomSampling()
            algorithm = NSGA2(pop_size=100, mutation=mutation, crossover=crossover, sampling=sampling)
            res = minimize(sequence_problem, algorithm, ('n_gen', 200), seed=1, verbose=False)
            span_sequence = []
            # res.F contains the pareto front
            # res.X contains the final population
            # res.X is a binary matrix where each row represents the assignment of servers to blocks
            # res.X[i][j] = 1 if server i is assigned to block j, 0 otherwise
            for i in range(len(res.X)):
                for j in range(len(res.X[i])):
                    if res.X[i][j] == 1:
                        span_sequence.append(self.state.sequence_info.spans_by_priority[i])
    ...
    def _build_inference_graph(
        ...
        *,
        ...
        print(f"\nClient server RTTS: {client_server_rtts}\n")
    ...
    def _make_sequence_with_max_throughput(self, start_index: int, end_index: int) -> List[RemoteSpanInfo]:
        ...
        print(f"\n function: _make_sequence_with_max_throughput --> \nClient server RTTS: {client_server_rtts}\n")
